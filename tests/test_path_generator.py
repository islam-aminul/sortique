...
from sortique.constants import DateSource, FileType, MAX_CONFLICT_ATTEMPTS
...
# ===========================================================================
# 3.  _build_category_path  (internal — no media-type prefix)
# ===========================================================================

class TestBuildCategoryPath:
    """Every documented category → folder mapping.

    _build_category_path always appends a year (``0000`` when absent).
    """
    ...
    def test_raw_no_year(self, gen):
        ...
        assert p == J("RAW", "Canon - EOS R5", "0000")
    ...
    def test_edited_no_year(self, gen):
        assert gen._build_category_path("Edited", None, None, None) == J("Edited", "0000")
    ...
    def test_export_no_year(self, gen):
        assert gen._build_category_path("Export", None, None, None) == J("Export", "0000")
    ...
    def test_originals_unknown_no_year(self, gen):
        ...
        assert p == J("Originals", "Unknown", "0000")
...
# ===========================================================================
# 4.  generate  (full path integration — with media-type prefix)
# ===========================================================================

class TestGenerate:
    """End-to-end path generation with media-type segregation."""
    ...
    def test_path_no_camera(self, gen, dest):
        dr = _dr(datetime(2024, 6, 1, 9, 0, 0))
        path = gen.generate(
            "Edited", "photo", ".png", dr, None,
            file_type=FileType.IMAGE,
        )
        expected = J(
            dest, "Images", "Edited", "2024",
    ...
    def test_path_no_date(self, gen, dest):
        path = gen.generate(
            "Screenshots", "capture", ".png", None, None,
            file_type=FileType.IMAGE,
        )
        expected = J(dest, "Images", "Screenshots", "capture.png")
        assert path == expected

    def test_path_no_metadata(self, gen, dest):
        """Collection without file_type falls back to legacy behaviour."""
        path = gen.generate("Collection", "mystery", ".dat", None, None)
        expected = J(dest, "Collection", "Unknown", "mystery.dat")
        assert path == expected

    def test_collection_with_file_type(self, gen, dest):
        """Collection with file_type skips sub-folder."""
        path = gen.generate(
            "Collection", "photo", ".jpg", None, None,
            file_type=FileType.IMAGE,
        )
        expected = J(dest, "Images", "Collection", "photo.jpg")
        assert path == expected

    def test_collection_legacy_jpg(self, gen, dest):
        """Collection without file_type adds Image sub-folder."""
        path = gen.generate("Collection", "photo", ".jpg", None, None)
        expected = J(dest, "Collection", "Image", "photo.jpg")
        assert path == expected

    def test_collection_legacy_mp4(self, gen, dest):
        path = gen.generate("Collection", "clip", ".mp4", None, None)
        expected = J(dest, "Collection", "Video", "clip.mp4")
        assert path == expected

    def test_collection_legacy_mp3(self, gen, dest):
    ...
    def test_export_with_date(self, gen, dest):
        ...
        path = gen.generate(
            "Originals", "IMG_001", ".jpg", dr, exif,
            file_type=FileType.IMAGE, is_export=True,
        )
        expected = J(
            dest, "Images", "Exports", "2024",
            "2024-03-15 14-30-00 -- Canon - EOS R5 -- IMG_001.jpg",
        )
        assert path == expected
    ...
    def test_export_no_date(self, gen, dest):
        path = gen.generate(
            "Originals", "IMG_001", ".jpg", None, None,
            file_type=FileType.IMAGE, is_export=True,
        )
        expected = J(dest, "Images", "Exports", "0000", "IMG_001.jpg")
        assert path == expected
    ...
    def test_documents_pdf(self, gen, dest):
        path = gen.generate(
            "Documents/PDF", "report", ".pdf", None, None,
    ...
    def test_raw_with_full_metadata(self, gen, dest):
        ...
        path = gen.generate(
            "RAW", "DSC_001", ".nef", dr, exif,
            file_type=FileType.IMAGE,
        )
        expected = J(
            dest, "Images", "RAW", "Nikon - Z8", "2023",
    ...
    def test_social_media_image(self, gen, dest):
        path = gen.generate(
            "Social Media", "IMG-WA001", ".jpg", None, None,
            file_type=FileType.IMAGE,
        )
        expected = J(dest, "Images", "Social Media", "IMG-WA001.jpg")
        assert path == expected

    def test_voice_notes(self, gen, dest):
        dr = _dr(datetime(2024, 2, 14, 18, 30, 0))
        path = gen.generate(
            "Voice Notes", "Recording_001", ".m4a", dr, None,
            file_type=FileType.AUDIO,
        )
        expected = J(
            dest, "Audio", "Voice Notes", "2024",
    ...
    def test_movies(self, gen, dest):
        path = gen.generate(
            "Movies", "movie", ".mp4", None, None,
            file_type=FileType.VIDEO,
        )
        expected = J(dest, "Videos", "Movies", "movie.mp4")
        assert path == expected

    def test_originals_no_date_uses_0000(self, gen, dest):
        exif = _exif(make="Canon", model="EOS R5")
        path = gen.generate(
            "Originals", "IMG_001", ".jpg", None, exif,
            file_type=FileType.IMAGE,
        )
        expected = J(
    ...
    def test_no_file_type_no_prefix(self, gen, dest):
        """Without file_type, no media-type prefix is added."""
        path = gen.generate("Other", "file", ".bin", None, None)
        expected = J(dest, "Other", "file.bin")
        assert path == expected
...
class TestSanitisationIntegration:
    ...
    def test_illegal_chars_in_original_name(self, gen, dest):
        path = gen.generate(
            "Screenshots", 'my<file>', ".png", None, None,
            file_type=FileType.IMAGE,
        )
        filename = os.path.basename(path)
        assert "<" not in filename
...
class TestEdgeCases:
    ...
    def test_empty_ext(self, gen, dest):
        path = gen.generate(
            "Collection", "file", "", None, None,
            file_type=FileType.IMAGE,
        )
        assert os.path.basename(path) == "file"

    def test_year_0000_when_no_date(self, gen, dest):
        exif = _exif(make="Canon", model="EOS R5")
        path = gen.generate(
            "Originals", "img", ".jpg", None, exif,
            file_type=FileType.IMAGE,
        )
        parts = path.split(os.sep)
        assert "0000" in parts

    def test_make_model_omitted_when_missing(self, gen, dest):
        dr = _dr(datetime(2024, 1, 1, 0, 0, 0))
        path = gen.generate(
            "Originals", "img", ".jpg", dr, None,
            file_type=FileType.IMAGE,
        )
        parts = path.split(os.sep)
        # Images / Originals / 2024 / filename — no make-model folder.
    ...
    def test_movies_no_year_subfolder(self, gen, dest):
        ...
        dr = _dr(datetime(2024, 1, 1, 0, 0, 0))
        path = gen.generate(
            "Movies", "movie", ".mp4", dr, None,
            file_type=FileType.VIDEO,
        )
        dirname = os.path.dirname(path)
        assert dirname == J(dest, "Videos", "Movies")
